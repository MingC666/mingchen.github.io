document.addEventListener("DOMContentLoaded", function () {
    ...
    function UpdateTransaction(event) {
        ...
        let oldPrice = parseFloat(cell.getAttribute("data-old"));
        cell.setAttribute("data-old", newPrice);

        let transArray = table.id === "diningTable" ? diningTrans : togoTrans;
        transArray[rowIndex - 1] = newPrice;

        let totalType = table.id === "diningTable" ? "diningTotal" : "togoTotal";
        window[totalType] = window[totalType] - oldPrice + newPrice;
        totalLabel.innerHTML = (parseFloat(diningTotalLabel.innerHTML) + parseFloat(togoTotalLabel.innerHTML)).toFixed(2);
